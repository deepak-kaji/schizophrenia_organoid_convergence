...
pb <- df %>%
  ...
  summarise(
    n_total = n(),
    n_Path2 = sum(Path == "Path2"),
    n_Path1 = sum(Path == "Path1"),
...
write.csv(coef_table, "/mnt/sdb/scz_meta_analysis_processed/trajectory_analyses/lmer_ei_pseudobulk_coefficients.csv", row.names = FALSE)
