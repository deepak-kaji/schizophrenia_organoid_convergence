...
SECOND_ROOT_DIR="/mnt/sda/scz_meta_analysis/khan_pasca_nature_med_2021/GEO_files"  # Add path to CellRanger data
...
## Loop through the second set of directories (CellRanger-style structure)
#for cellranger_dir in $SECOND_ROOT_DIR/*; do
#    if [ -d "$cellranger_dir" ]; then
#        # Create a cellbender output directory inside /mnt/lacie/scz_meta_analysis/cellbender for this dataset
#        output_base_dir="$OUTPUT_DIR/$(basename "$cellranger_dir")"
#        mkdir -p "$output_base_dir"
#
#        # Loop through each subdirectory (i.e., GSM directories)
#        for gsm_dir in "$cellranger_dir"/*; do
#            if [ -d "$gsm_dir" ]; then
#                # Define the directory containing the .mtx and other files
#                matrix_dir="$gsm_dir"
#
#                # Check if the .mtx file exists in the directory
#                if ls "$matrix_dir"/*.mtx.gz &>/dev/null; then
#                    echo "Running CellBender on $matrix_dir"
#
#                    # No need to rename features.tsv to genes.tsv for CellRanger data
#
#                    # Create a specific output directory for this particular library
#                    library_output_dir="$output_base_dir/$(basename "$gsm_dir")"
#                    mkdir -p "$library_output_dir"  # Ensure the subdirectory exists
#		    
#                    # Get To The Output Directory So It Drops The Checkpoint There 
#		    cd $library_output_dir
...
#                        remove-background --cuda \
#                        --input "$matrix_dir" \
#                        --output "$library_output_dir/$(basename "$matrix_dir")_cellbender_output.h5" || echo "Error: CellBender run failed for $matrix_dir" >> /mnt/sdb/scz_meta_analysis_processed/cellbender_error_log.txt
#                else
#                    echo "No .mtx.gz file found in $matrix_dir"
#                fi
#            fi
#        done
#    fi
