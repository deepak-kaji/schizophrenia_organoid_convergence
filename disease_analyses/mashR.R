...
common_genes <- Reduce(intersect, list(dge_22q11$ID, dge_nrxn1$ID, dge_idiopathic$ID,
				       dge_purcell$ID, dge_walsh_15q13$ID))
...
# -----------------------
write.csv(pm, file.path(outdir, "mash_posterior_mean.csv"))
write.csv(lfsr, file.path(outdir, "mash_lfsr.csv"))
write.csv(orig, file.path(outdir, "mash_logFC_original.csv"))
write.csv(post_sd, file.path(outdir, "mash_posterior_sd.csv"))
write.csv(neg_prob, file.path(outdir, "mash_negative_prob.csv"))
...
write.csv(
  ...
  file.path(outdir, "mash_sig_any_gene.csv"),
  row.names = FALSE
)

write.csv(
  data.frame(celltype = names(sig_per_celltype), n_sig = sig_per_celltype),
  file.path(outdir, "mash_sig_per_celltype.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(outdir, "gene_missingness.csv"),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(outdir, "celltype_missingness.csv"),
  row.names = FALSE
)
