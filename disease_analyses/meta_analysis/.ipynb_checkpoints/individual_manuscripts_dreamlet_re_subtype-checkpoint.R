...
meta_analysis_heterogeneity <- function(x, method = "FE", group = c("ID", "assay"), control = list(maxiter = 2000)) {
  ...
  f <- function(beta, se, method) {
    if (method == "RE2C") {
      ...
      fit <- rma(yi = beta, sei = se, method = method, control = control)
...
res.nrxn1 <- meta_analysis_heterogeneity(dataset_nrnx1, method = 'REML')
res.22q11 <- meta_analysis_heterogeneity(dataset_22q11, method = 'REML')
res.idiopathic <- meta_analysis_heterogeneity(dataset_idiopathic, method = 'REML')

# Save as CSV

CSV_PATH <- '/mnt/sdb/scz_meta_analysis_processed/dge_signatures/dreamlet_dges/disease_analyses/'

write.csv(res.nrxn1, paste0(CSV_PATH, 'random_dreamlet_nrxn1_combined_subtype.csv'), row.names = FALSE)
write.csv(res.22q11, paste0(CSV_PATH, 'random_dreamlet_22q11_combined_subtype.csv'), row.names = FALSE)
write.csv(res.idiopathic, paste0(CSV_PATH, 'random_dreamlet_idiopathic_combined_subtype.csv'), row.names = FALSE)
