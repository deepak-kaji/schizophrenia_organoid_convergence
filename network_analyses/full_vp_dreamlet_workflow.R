...
ggsave(plot_voom_fig, file='/mnt/sdb/scz_meta_analysis_processed/dge_signatures/dreamlet_dges/disease_analyses/plot_voom_scenic.png', dpi=500)

# --- Step 3: Fit variance partition model ---

vp.lst <- fitVarPart(res.proc.vp, formula_full)

write.csv(vp.lst, file = "/mnt/sdb/scz_meta_analysis_processed/dge_signatures/dreamlet_dges/disease_analyses/variance_partition_long_scenic.csv", row.names = FALSE)

# Optional: Visualize variance explained
plot_varpart <- plotVarPart(vp.lst)
ggsave(plot_varpart, file='/mnt/sdb/scz_meta_analysis_processed/dge_signatures/dreamlet_dges/disease_analyses/visualize_variance_full_scenic.png', dpi=500)
...
write.csv(dge_22q11, paste0(CSV_PATH, 'dreamlet_22q11_dge_scz_scenic_results.csv'), row.names = FALSE)
write.csv(dge_NRXN1, paste0(CSV_PATH, 'dreamlet_NRNXN1_dge_scz_scenic_results.csv'), row.names = FALSE)
write.csv(dge_3q29, paste0(CSV_PATH, 'dreamlet_3q29_dge_scz_scenic_results.csv'), row.names = FALSE)
write.csv(dge_15q13, paste0(CSV_PATH, 'dreamlet_15q13_dge_scz_scenic_results.csv'), row.names = FALSE)
write.csv(dge_Idiopathic, paste0(CSV_PATH, 'dreamlet_Idiopathic_dge_scz_scenic_results.csv'), row.names = FALSE)
